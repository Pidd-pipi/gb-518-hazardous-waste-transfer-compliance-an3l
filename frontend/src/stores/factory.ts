...
export class EntityStore {
  ...
  async transition(path: string, item: DomainRecord, status: string, extra: { reason?: string; receivedWeightKg?: number | null; weightDiffReason?: string } = {}) { this.patch({ loading: true }); try { await request<DomainRecord>(`/${path}/${item.id}/transition`, { method: 'POST', body: JSON.stringify({ status, expectedVersion: item.version, reason: extra.reason || '前端工作台人工确认', receivedWeightKg: extra.receivedWeightKg ?? null, weightDiffReason: extra.weightDiffReason || '' }) }); await this.load(path); } catch (error) { this.patch({ loading: false, error: error instanceof Error ? error.message : String(error) }); throw error; } }
