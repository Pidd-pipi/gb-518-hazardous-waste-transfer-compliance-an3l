...
import { formatDate, formatWeightVariance } from '../utils/format';
  ...
  template: `
    <main class="workspace" *ngIf="store.state$ | async as state">
      ...
      <section class="table-shell">
        <table>
          <thead><tr><th>编码</th><th>名称</th><th>状态</th><th>业务凭证</th><th>风险</th><th>责任人</th><th>指标</th><th *ngIf="isManifestPage()">实收重量</th><th *ngIf="isManifestPage()">差异原因</th><th>更新时间</th><th>操作</th></tr></thead>
          <tbody>
            <tr *ngFor="let item of state.items; trackBy: trackById">
              ...
              <td *ngIf="isManifestPage()">
                <ng-container *ngIf="item.receivedWeightKg != null; else noReceipt">
                  {{ item.receivedWeightKg }} kg<small>{{ formatVariance(item.weightVarianceKg) }}</small>
                </ng-container>
                <ng-template #noReceipt><span class="muted">未签收</span></ng-template>
              </td>
              <td *ngIf="isManifestPage()"><span class="domain-detail">{{ item.varianceReason || '—' }}</span></td>
              ...
              <td class="actions">
                <ng-container *ngIf="canTransition()">
                  ...
                  <button *ngIf="canReceive(item)" class="table-action" (click)="openReceive(item)">签收</button>
                </ng-container>
                <span *ngIf="!canTransition() || (transitions(item).length === 0 && !canReceive(item))" class="muted">{{ auth.hasMinimumRole('operator') ? '流程结束' : '只读' }}</span>
              </td>
            </tr>
            <tr *ngIf="!state.items.length && !state.loading"><td [attr.colspan]="isManifestPage() ? 11 : 9" class="empty">暂无记录</td></tr>
          </tbody>
        </table>
      ...
      <app-confirm-dialog [open]="!!receivePending" title="联单签收" [disabled]="receiveInvalid()" (cancel)="closeReceive()" (confirm)="confirmReceive()">
        <p>计划重量 <strong>{{ receivePending?.quantityKg }} kg</strong>，签收将记录实收重量、与计划的差值和签收时间，保存后不可改写。</p>
        <label class="field">
          <span>实收重量 (kg)</span>
          <input matInput type="number" min="0" step="0.1" [(ngModel)]="receiveWeight" placeholder="现场过磅重量，必填" />
        </label>
        <p *ngIf="receiveVariance() !== null" class="muted">与计划差值：{{ formatVariance(receiveVariance()) }}</p>
        <ng-container *ngIf="receiveOverweight()">
          <label class="field">
            <span>差异原因（超出计划 5% 必填）</span>
            <input matInput [(ngModel)]="receiveReason" placeholder="说明实收与计划的差异原因" />
          </label>
          <p class="alert" role="alert">实收重量超出计划 5%，确认后联单将转为驳回。</p>
        </ng-container>
      </app-confirm-dialog>
    </main>
  `
...
export class EntityPageComponent implements OnInit {
  ...
  receivePending: DomainRecord | null = null;
  receiveWeight: number | null = null;
  receiveReason = '';
  private manifestStatusByCode = new Map<string, string>();

  constructor(private readonly changeDetector: ChangeDetectorRef) {}

  async ngOnInit(): Promise<void> {
    await this.load();
    if (this.config.key === 'complianceCheck') { await this.loadManifestStatuses(); }
  }
  ...
  canReceive(item: DomainRecord): boolean { return this.isManifestPage() && item.status === 'in_transit' && this.auth.hasMinimumRole('operator'); }

  transitions(item: DomainRecord): readonly string[] {
    const targets = TRANSITIONS[this.config.key]?.[item.status] ?? [];
    if (this.config.key !== 'complianceCheck') { return targets; }
    const manifestStatus = item.manifestCode ? this.manifestStatusByCode.get(item.manifestCode) : undefined;
    return targets.filter((target) => {
      // 通过必须等联单签收；待决定的核验只有已驳回联单才能直接升级复核。
      if (target === 'pass') { return manifestStatus === undefined || manifestStatus === 'received'; }
      if (target === 'escalated' && item.status === 'pending') { return manifestStatus === undefined || manifestStatus === 'rejected'; }
      return true;
    });
  }
  ...
  pageDescription(): string {
    const descriptions: Record<string, string> = {
      ...
      transferManifest: '关联产废单位与承运方，签收时记录实收重量与差异。',
      complianceCheck: '基于已签收联单的证据形成不可回退的核验决定。'
    };
  ...
  formatVariance(value?: number | null): string { return formatWeightVariance(value); }
  receiveVariance(): number | null {
    if (this.receivePending == null || this.receiveWeight == null) { return null; }
    return this.receiveWeight - (this.receivePending.quantityKg ?? 0);
  }
  receiveOverweight(): boolean {
    return this.receivePending != null && this.receiveWeight != null
      && this.receiveWeight > (this.receivePending.quantityKg ?? 0) * RECEIVE_TOLERANCE;
  }
  receiveInvalid(): boolean {
    if (this.receiveWeight == null || this.receiveWeight <= 0) { return true; }
    return this.receiveOverweight() && this.receiveReason.trim() === '';
  }
  ...
  openReceive(item: DomainRecord): void { this.receivePending = item; this.receiveWeight = null; this.receiveReason = ''; this.changeDetector.detectChanges(); }
  closeReceive(): void { this.receivePending = null; this.changeDetector.detectChanges(); }
  ...
  async confirmReceive(): Promise<void> {
    if (!this.receivePending || this.receiveInvalid() || this.receiveWeight == null) return;
    try {
      await this.store.receive(this.config.path, this.receivePending, this.receiveWeight, this.receiveReason.trim());
  ...
  private async loadManifestStatuses(): Promise<void> {
    try {
      ...
      this.manifestStatusByCode = new Map(result.data.map((manifest) => [manifest.code, manifest.status]));
    } catch { this.manifestStatusByCode = new Map(); }
    this.changeDetector.detectChanges();
  }
}
