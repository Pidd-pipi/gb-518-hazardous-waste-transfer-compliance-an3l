...
func (h *TransferManifestHandler) Register(group *gin.RouterGroup) {
	...
	resource.POST("/:id/receive", middleware.RequireMinimumRole(model.RoleOperator), h.receive)
...
func (h *TransferManifestHandler) receive(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.ReceiveManifest
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
	item, err := h.service.Receive(c.Request.Context(), id, input, actorFromContext(c), requestIDFromContext(c))
	if err != nil {
		handleError(c, err)
		return
	}
